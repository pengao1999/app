		  ...
		  <form @submit="formBindsubmit">
			<view class='column-box'>
			  ...
			  <view class='col'>
				...
				<input name="address" class='input' placeholder='请填写详细地址' :value='myleader.address'></input>
			  </view>
			  <view class='col'>
				<text class='name'>小区名称</text>
				<input name="community" class='input' placeholder='请填写小区名称' :value="myleader.community"></input>
			  </view>
			  <view class='col'>
				<text class='name'>姓名</text>
				<input name="name" class='input' placeholder='请填写您的姓名' :value="myleader.name"></input>
			  </view>
			  <view class='col'>
				<text class='name'>联系方式</text>
				<input name="tel" class='input' placeholder='请填写联系方式' :value="myleader.tel"></input>
			  </view>
			</view>

			<view class='provision' v-if="!myleader.check_state|myleader.check_state==3">
			  <checkbox-group>
				<label class="checkbox">
				  <checkbox :value="hasread" :checked="hasread" @click.stop="checkReadProtocal(hasread)" class="round" style="margin-right: 10rpx;" />已阅读
				</label>
				<text  @tap.stop="toProtocol">《团长申请协议》</text>
			  </checkbox-group>
			</view>

			<button formType="submit" class='submit' v-if="!myleader.check_state">提交申请</button>
			<button class="submit gray" v-if="myleader.check_state==1">待审核</button>
			<button class="submit" formType="submit" v-if="myleader.check_state==3">重新提交</button>
		  </form>
		  <view class='reason' v-if="myleader.check_state==3">审核失败原因：{{myleader.fail_reason}}</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			toProtocol: function() {
				uni.navigateTo({
					url: '/pkUserInfo/info/protocolDetails?id=13'
				})
			},
			initPage: function() {
				this.getMyLeader()
			...
			getMyLeader: function() {
				...
				self.$http.post(Config.URLS.getApplyLog, data).then((res) => {
					self.toast(true)
					res = res.data
					if (res.code === 0) {
						if (res.data !== null) {
							self.myleader = Object.assign({}, res.data)
			...
			selectAddress: function() {
				...
				uni.chooseLocation({
					success(e) {
						self.myleader = Object.assign({},self.myleader, {
							address: e.address
						})
			...
			formBindsubmit: function(e) {
				...
				self.$http.post(Config.URLS.applyLeader, data).then((res) => {
					res = res.data
					if (res.code === 0) {
						self.toast({
							type:'success',
						...
						self.myleader = Object.assign({}, self.myleader, res.data.data)
					}
				})
			},
			// 错误隐藏
