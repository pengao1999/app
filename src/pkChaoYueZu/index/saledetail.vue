		...
		<scroll-view class="scrollView" scroll-y="true" :style="'height:'+windowHeight+'px;'">
			<view class="bg-white">
				...
				<view class="align-end row-wrap row-container" style="width: 690upx;" v-if="periodsConfig.periods">
					<!-- <view class="text-goods-price" v-if="associations.length >0?associations[0].is_selling !== 1?true:false:false">
					...
					<view class="text-goods-price">
						<text class="num" v-if="goods.original_price">¥{{goods.original_price || 0 }}元</text>
						<text class="num" v-else>计算中...</text>
					</view>
					...
					<!-- <block v-if="associations.length >0?associations[0].is_selling !== 1?true:false:false">
          ...
          <text class="row-right">></text>
			...
			<scroll-view scroll-x class="bg-white nav text-center">
				...
				<!-- <view class="cu-item" :class="2==TabCur?'text-red cur':''" @click="TabCur=2" v-if="associations.length >0?associations[0].is_selling !== 1?true:false:false">
					租赁必读
				</view> -->
			</scroll-view>
			...
			<view class="fixed-btn row-container" style="font-size: 40rpx;width: 100%;height: 100rpx; background: white">
				...
				<!-- <view class="bottom-btn" @click="zulin" v-if="associations.length >0?associations[0].is_selling !== 1?true:false:false">
					立即租
				</view> -->
					...
					<scroll-view class="specification-wrapper-content" scroll-y="true">
						<view class="specification-header">
							...
							<view class="specification-right">
								<view class="price-content">
									...
									<text class="price" v-if="periodsConfig.periods">{{ (selectshop.price/ (is_selling == 0? periodsConfig.periods:-periodsConfig.periods) + selectshop.price*goods.rent_interest_rate*0.01).toFixed(2) || 0 }}<text
										 style="font-size: 30upx;" v-if="associations.length >0?associations[0].is_selling !== 1?true:false:false">/{{periodsConfig.unit}}</text></text>
								</view>
						...
						<view class="specification-content">
							...
							<view class="specification-item coloum-container" v-if="associations.length>0">
								<view class="row-container align-center" @click="toArticle('e53ccf867415436e97f2a44b0f5ed273', '关于意外保障')">
									<view class="item-title">增值服务（必选）</view>
...
<script>
	...
	import stepper from '../../components/yx_view/stepper'
			...
			toOrder: function() {
				...
				self.$Phttp.get(CreditConfig.REQUEST_URLS.creditStatus, data).then(res => {
					...
					if (res.code !== 200) {
						...
						uni.navigateTo({
							url: "/pkChaoYueZu/order/create?way=2&rentCode=" + zuqiCode + '&dims=' + dims
						});
					}
				}).catch(err => {
