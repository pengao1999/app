<template>
  <view class="page">
    ...
    <view class="row-wrap">
      <view class="row-text">联系方式</view>
      <input placeholder="手机/邮箱" class="row-input" v-model="sendDate.contact" />
    </view>
    <!-- <view class="feedback-title">
      <text>QQ/邮箱</text>
    </view>
    ...
    <!-- <view class="feedback-title feedback-star-view">
      ...
      <view class="feedback-star-view">
        <text
          ...
          :key="key"
          :class="key < sendDate.score ? 'active' : ''"
          @tap="chooseStar(value)"
        ></text>
      </view>
    </view>-->
    <button type="default" class="bottom-btn" @tap="send">提交</button>
    <!-- <view class="feedback-title">
...
export default {
  ...
  methods: {
    ...
    send() { //发送反馈
      let self = this
      let imgs = self.imageList.map((value, index) => {
        return {
          name: "image" + index,
          uri: value
        }
      })
      console.log(JSON.stringify(self.sendDate));
      console.log(imgs)
      uni.uploadFile({
        url: "/",
        files: imgs,
        formData: self.sendDate,
        success: (res) => {
        ...
        complete() {
          ...
          setTimeout(function(){uni.switchTab({url: '/pkChaoYueZu/profile/profile'})}, 2500)
        }
      });
    }
  }
