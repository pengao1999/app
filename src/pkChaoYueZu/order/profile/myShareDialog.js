...
nvImageMenu.draw([
	...
	},
	{
		tag: 'img',
		id: 'imgqq',
		src: '/static/sharemenu/qq-icon.png',
		position: {
			top: top1,
			left: left4,
			width: iconWidth,
...
nvImageMenu.addEventListener("click", function (e) { //处理底部图标菜单的点击事件，根据点击位置触发不同的逻辑
	...
	if (e.screenY > plus.screen.resolutionHeight - 44 - 34) { //点击了底部取消按钮
		...
		if (iClickIndex >= 0 && iClickIndex <= 6) { //处理具体的点击逻辑，此处也可以自行定义逻辑。如果增减了按钮，此处也需要跟着修改
			...
			console.log('iClickIndex', iClickIndex)
			if (iClickIndex === 0 || iClickIndex === 1 || iClickIndex === 2 || iClickIndex === 3) {
				console.log('/pages/index/product?id=' + id + '&leader_id=' + leader_id + '&activity_id=' + activity_id + '&activity_endtime=' + activity_endtime + "&i=" + globalI)
				console.log('strShareUrl:' + strShareUrl + ' strShareTitle:' + strShareTitle + ' strShareSummary:' + strShareSummary + ' strShareImageUrl:' + strShareImageUrl)
				uni.share({
					...
					miniProgram: {
						id: 'gh_ae6e1c57380d',
						path: '/pages/index/product?id=' + id + '&leader_id=' + leader_id + '&activity_id=' + activity_id + '&activity_endtime=' + activity_endtime + "&i=" + globalI,
						type: 0,
						webUrl: web_url,
					},
...
export default {
	...
	methods: {
		...
		setShareMessage(url, title, summary, imageUrl, good_id, lid, aid, at, i) {
			// #ifdef APP-PLUS
			strShareUrl = url
			strShareTitle = title
