		...
		<view class="tabs-container" :style="{
			...
			<scroll-view id="tabs-scroll" scroll-x :scroll-left="scrollLeft" class="tabs-scroll" scroll-with-animation>
				<view class="tabs-scroll-box" :style="{
					...
					<view id="line" class="line round" :style="{
						'height': lineHieght,
			...
			<block v-if="String(hasRefresh) === 'true'">
				<view class="refresh-container" id="refresh-container" @touchstart="QSREFRESHWXS.touchstart" @touchmove="QSREFRESHWXS.touchmove"
				 ...
				 :data-refreshshow="refreshShow" :refreshstatus="refreshStatus">
					<view class="refresh-image-container" :class="{'show': (String(refreshShow) == 'true')}" :style="{
						'height': refreshDistance + 'px'
					}">
						<image id="refreshImage" v-if="refreshImage" :src="refreshImage" mode="scaleToFill" class="refresh-image" :class="{'isRefreshing': (String(readyRefresh) == 'false')}"></image>
					...
					<swiper id="swiper" class="swiper" :style="{
					 ...
					 @animationfinish="QSTABSWXS.animationfinish">
						<swiper-item v-for="(item, index) in tabs" :key="index" class="swiper-item" :style="{
							...
							<block v-if="item.type === 'zulin'">
								<zulinTab ref="QSTabsWxsRef" :readyRefresh="getCurrent===index?Boolean(readyRefresh):false" :show="String(readyRefresh) === 'true'?
								(getCurrent===index):
								(((index-getCurrent)<=1 && (index-getCurrent)>=-1)?
								true:false)"
								 :type="type" :current="getCurrent" :tab="item" :index="index" @refreshEnd="setRefreshStatus(4)"></zulinTab>
							</block>
							<block v-if="item.type === 'yueji'">
								<yuejiTab ref="QSTabsWxsRef" :readyRefresh="getCurrent===index?Boolean(readyRefresh):false" :show="String(readyRefresh) === 'true'?
								(getCurrent===index):
								(((index-getCurrent)<=1 && (index-getCurrent)>=-1)?
								true:false)"
								 :type="type" :current="getCurrent" :tab="item" :index="index" @refreshEnd="setRefreshStatus(4)"></yuejiTab>
							</block>
							<block v-if="item.type === 'weixiu'">
								<weixiuTab ref="QSTabsWxsRef" :readyRefresh="getCurrent===index?Boolean(readyRefresh):false" :show="String(readyRefresh) === 'true'?
								(getCurrent===index):
								(((index-getCurrent)<=1 && (index-getCurrent)>=-1)?
								true:false)"
								 :type="type" :current="getCurrent" :tab="item" :index="index" @refreshEnd="setRefreshStatus(4)"></weixiuTab>
							</block>
							<block v-if="item.type === 'huishou'">
								<HuiShouTab ref="QSTabsWxsRef" :readyRefresh="getCurrent===index?Boolean(readyRefresh):false" :show="String(readyRefresh) === 'true'?
								(getCurrent===index):
								(((index-getCurrent)<=1 && (index-getCurrent)>=-1)?
								true:false)"
								 :type="type" :current="getCurrent" :tab="item" :index="index" @refreshEnd="setRefreshStatus(4)"></HuiShouTab>
							</block>
							<block v-if="item.type === 'zhishou'">
								<ZhiShouTab ref="QSTabsWxsRef" :readyRefresh="getCurrent===index?Boolean(readyRefresh):false" :show="String(readyRefresh) === 'true'?
								(getCurrent===index):
								(((index-getCurrent)<=1 && (index-getCurrent)>=-1)?
								true:false)"
								 :type="type" :current="getCurrent" :tab="item" :index="index" @refreshEnd="setRefreshStatus(4)"></ZhiShouTab>
							</block>
						</swiper-item>
					</swiper>
				</view>
...
<script>
	...
	import zulinTab from './components/zulin-tab.vue'
	import yuejiTab from './components/yueji-tab'
	import weixiuTab from './components/weixiu-tab.vue'
	import HuiShouTab from './components/huishou-tab.vue'
	import publicProps from './js/publicProps.js';
	import ZhiShouTab from './components/zhishou-tab.vue'
	const {
		windowWidth
