<template>
  <div class="main-page bg-white coloum-container">
    ...
    <div class="Card-40 info-card" v-for="(card, index) in cards" :key="index" style="margin-left: 40upx;margin-top: 55upx;">
      <image class="info-card lf" src="/static/zulin/profile/jingjiren/bank-card.png" />
    ...
    <button
      class="cu-btn bg-gradual-blue btn fixed-bttom safe-area round"
      @click="addBankCard"
    >添加银行卡</button>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        self.$Phttp.get(Config.JAVA_SAAS_URLS.getBankCardList, postData).then(res => {
          ...
          if (res.code === 0) {
            res.data.forEach((card, index)=>{
              card.hideNumber = self.changeStr(card.bank_card_number, 4, "*******")
            })
            self.cards = res.data
          }
        })
      ...
      changeStr(str,index,changeStr){ // 截取后 index 位
        return changeStr + str.substring(str.length-4);
      }
    }
  }
</script>
