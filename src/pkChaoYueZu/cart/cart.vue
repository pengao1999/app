<template>
  <view class="container">
    <view class="no-data" v-if="cart.length<1">
      <image class="bg" :lazy-load="true" src="/static/bg_cart.png" />
      <text class="desc">购物车空空如也~</text>
      <text class="to-buy" @tap="toBuy">去选品</text>
    </view>
    ...
    <scroll-view scroll-y="true" class="items" v-else>
      <block v-for="(item,index) in cart" :key="index">
        <view class="item">
          <checkbox
            :checked="item.status"
            @tap.stop="selectGood(index)"
            class="round"
            style="flex-shrink: 0; zoom: 0.8;"
          />
          <image class="image" :lazy-load="true" :src="image_root+item.pic" />
          <view class="content">
          ...
          <image
            class="delete"
            :lazy-load="true"
            @tap.stop="deleCarts(item.id, index)"
    ...
    <view class="ad-box" v-if="banner">
      <image class="ad" :lazy-load="true" :src="image_root+banner.pic" @tap="adClick" />
    </view>
...
export default {
  ...
  methods: {
    ...
    getCart: function () {
      ...
      self.$http.post(Config.URLS.getCarts, data).then((res) => {
        self.toast(true)
        res = res.data
    ...
    deleCarts: function (cart_id, idx) {
      let self = this
      uni.showModal({
        confirmColor: "#1dc422",
        title: '提示',
        ...
        success: (res) => {
          if (res.confirm) {
            ...
            self.$http.post(Config.URLS.delCart, data).then((res) => {
              self.toast(true)
              res = res.data
              if (res.code === 0) {
                self.toast({
                ...
                if (self.cart[idx].attr_names === "") {
                  self.addIndexRefresh({
                    goods_id: self.cart[idx].goods_id,
                    cart_num: 0
                  })
                  self.addCategoryRefresh({
                    goods_id: self.cart[idx].goods_id,
...
.items .item .bottom .num-box .reduce,
.items .item .bottom .num-box .add {
  width: 40rpx;
  height: 40rpx;
  float: left;
}
