<template>
	<view :class="setCollapseCellClass" :hover-class="disabled === true || disabled === 'true' ? '' : 'uni-collapse-cell--hover'">
		...
		<view class="uni-collapse-cell__content" :class="animation==='outer' ? 'uni-collapse-cell--animation' : ''" >
			<view v-if="isOpen==='true' || isOpen=== true" :class="setContClass" :id="elId">
				<view class="item" v-for="(city, cIndex) in citys" @tap="chooseCity(city.label)" :key="cIndex">
					{{city.label}}
				</view>
			</view>
		</view>
