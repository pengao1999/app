<template>
	<view class="mosowe-swiper" :style="'height:'+height+'rpx'">
		<swiper class="uni-swiper" :indicator-dots="indicator === 'dots'" :indicator-color="indicatorColor"
		 :indicator-active-color="indicatorActiveColor" :autoplay="autoplay" :current="active" :interval="interval" :duration="duration"
		 :vertical="vertical" :disable-touch="touchable" :previous-margin="canPyramid ? pyramidMargin : ''" :next-margin="canPyramid ? pyramidMargin : ''"
		 :display-multiple-items="canPyramid ? 1 : itemNums" circular @change="bannerChange">
			<!-- 非微信小程序 -->
			<!-- #ifndef MP-WEIXIN -->
			<swiper-item class="item" v-for="(item,index) of lists" :key="index" :class="canPyramid && active !== index ? 'swiper-pyramid' : canPyramid && active === index ? 'swiper-active' : ''"
			...
			<block v-if="touchable">
				<swiper-item class="item" v-for="(item,index) of lists" :key="index" :class="canPyramid && active !== index ? 'swiper-pyramid' : canPyramid && active === index ? 'swiper-active' : ''"
				 @touchstart.stop="WXAPP_bannerTouch" @touchend="WXAPP_bannerTouchEnd({item,index})">
					<!-- 仅图片展示 -->
