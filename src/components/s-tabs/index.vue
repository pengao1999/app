				...
				<div class="scroll-view">
					<div class="s-tab-nav-view">
						<template v-if="!slotTitle">
							<div v-for="(item,index) of navInfoList" :class="['s-tab-nav',{'is-disabled':item.disabled},{'is-active':active==index},{'is-fontweight':active==index}]"
							 :style="navWidth+'color:'+(active==index?activeColor:color)"
							 :key="index" @click="navClick(index,item)">{{item.title}}</div>
						</template>
						<slot v-else></slot>
					</div>
					<div v-if="line" class="s-tab-line" :style="{
              width:lineWidth+'px',
