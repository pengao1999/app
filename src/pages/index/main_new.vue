  ...
  <view>
    <view class="index-content" v-if="loadingPageSuccess">
      ...
      <index-swiper
        v-if="showSwiper"
        :banners="banners"
        :customFormat="swiperNode.format"
        @changeImg="changeSwiperImg"
        @bannerTap="bannerTap"
      ></index-swiper>
      ...
      <index-menu
        v-if="showComponents"
        :menuData="menuData"
        :menuFormat="menuNode.format"
        :group="group"
        :isNewcomers="isNewcomers"
        :pickData="pickData"
        :activity_key="activity_key.toString()"
        :activity_endtime="activity_endtime.toString()"
        :userInfo="userInfo"
        :globalI="globalI"
        @clickNavList="clickNavList"
        @setNewPageTag="setNewPageTag"
      >
      </index-menu>
      ...
      ></product-selector>
      <my-ad-diaolog
        :propData="adDialogData"
        @closeAdDialog="closeAdDialog"
        @adClick="bannerTap(adDialogData)"
      ></my-ad-diaolog>
    </view>

    <view v-if="showComponents">
      <!--  #ifdef  MP-WEIXIN || H5 -->
      <my-wx-login
        :propData="loginDialogData"
        @handleClose="handleClose"
        @getUserInfoSuccess="getUserInfoSuccess"
        @getUserInfoH5="getUserInfoH5"
      ></my-wx-login>
      <!--  #endif -->
      <position-dialog
      ...
      <service-center-dialog
        :showPositionDialog="showCenterDialog"
        @enter="toArea"
      ></service-center-dialog>
      <coupon-dialog
        :bannerShow="showCouponDialog"
        :bannerStyle="couponDialogBackground"
        :coupons="coupons"
...
import * as indexPageCustomization from "@/pages/customization/index-page-customization.json";
import IndexTopAddress from './components/IndexTopAddress.vue'
...
export default {
  ...
  watch: {
    ...
    isGettingNextPageData: {
      handler() {
        if (this.isGettingNextPageData && this.inBottom) {
  ...
  onLoad(options) {
    ...
    if (options !== undefined && Util.containsKey(options, 'user_key')) {
      self.setUserInfo({
    ...
    setTimeout(() => {
      self.showAddress = self.checkExistenceOfModule('address')
    })
    setTimeout(() => {
      self.showSwiper = self.checkExistenceOfModule('swiper')
    }, 80)
    setTimeout(() => {
      self.showComponents = self.checkExistenceOfModule('menu')
    }, 120)
    setTimeout(() => {
      self.showGoods = self.checkExistenceOfModule('goodList')
    }, 160)

    const navNode = self.getCustomNodeInModule('nav')
    uni.setNavigationBarTitle({ title: navNode.element.title })
    uni.setNavigationBarColor({ frontColor: '#ffffff', backgroundColor: navNode.format.background })

    self.addressNode = self.getCustomNodeInModule('address')
    self.swiperNode = self.getCustomNodeInModule('swiper')
    self.menuNode = self.getCustomNodeInModule('menu')
  ...
  methods: {
    ...
    getCustomNodeInModule(moduleName) {
      let resultNode = {}
      if (indexPageCustomization[moduleName]) {
        resultNode = indexPageCustomization[moduleName]
      }
      return resultNode
    },
    checkExistenceOfModule(moduleName) {
      return indexPageCustomization[moduleName]
    },
    ...
    getStoreInfo: function () {
      let self = this
      self.$http.post(Config.URLS.getStoreInfo, {}).then(res => {
        res = res.data
        if (res.code === 0) {
    ...
    getIbyLocation: function (location) {
      let self = this
      self.$http.post(Config.URLS.getIbyLocation, location).then((res) => {
        res = res.data
        if (res.code === 0) {
          if (res.data.uniacid === "") {
            // self.showCenterDialog = true
    ...
    setGroupIfHasLeaderId: function (leader_key) {
      ...
      self.$http.post(Config.URLS.getMyLeader, data).then((res) => {
        res = res.data
    ...
    getGoods: function (catType = CURRENT_CAT_TYPE, page = 1, isForce = false) {
      ...
      self.$http.post(Config.URLS.getGoods, data).then((res) => {
        ...
        if (res.data.acts.length !== 0)
          self.cutDown(res.data.acts[0].end_time)
        self.currentPage++;
      }).catch(err => {
        if (self.isError) return
        self.isError = true
    ...
    getBanner: function () {
      ...
      self.$http.post(Config.URLS.getBanner, {}).then((res) => {
    ...
    isShowNewcomers: function () {
      ...
      self.$http.post(Config.URLS.isNewcomers, data).then((res) => {
        res = res.data
        if (res.code === 0) {
    ...
    getNewOrder: function () {
      ...
      self.$http.post(Config.URLS.getNewBuyer, data).then((res) => {
    ...
    getCoupons: function () {
      ...
      self.$http.post(Config.URLS.getCoupons, data).then((res) => {
        res = res.data
        if (res.code === 0) {
          self.toast({
            type: 'success',
    ...
    getCouponList: function () {
      ...
      self.$http.post(Config.URLS.couponDialog, data).then((res) => {
    ...
    checkUpdate(widgetInfo) {
      ...
      self.$http.post(Config.URLS.checkUpdate, postData).then((res) => {
  ...
  onShareAppMessage() {
    return {
      ...
      path: '/pages/index/main?user_key=' + this.userInfo.user_key
    }
  },
  onPullDownRefresh() {
