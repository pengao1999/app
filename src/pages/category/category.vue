<template>
	<view>
		...
		<view class='top'>
		  <view class='search'>
			<input class='keyword' type='text'  @tap="toSearch" placeholder='请输入商品名称/关键字'  placeholder-class='placeholder' disabled="true"></input>
			<image class='button' @tap="toSearch" src='../../static/ic_search.png'></image>
		  </view>
		</view>
		<!-- 搜索框结束 -->
	...
	export default {
		...
		methods: {
			...
			buy: function(item) {
				...
				self.$Phttp.post(Config.JAVA_SAAS_URLS.goodDetail, {goods_key: item.goods_key}).then(res => {
					res = res.data
					if (res.code === 0) {
